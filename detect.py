...
def permutation_test(tokens,vocab_size,key,n,k,n_runs=100,max_seed=100000):
    rng = mersenne_rng(key)
    xi = np.array([rng.rand() for _ in range(n*vocab_size)], dtype=np.float32).reshape(n,vocab_size)
    test_result = detect(tokens,n,k,xi,vocab_size)

    unique_tokens, _ = np.unique(tokens, return_inverse=True)
    eff_vocab_size = np.max(unique_tokens) + 1

    p_val = 0
    for run in range(n_runs):
        xi_alternative = np.random.rand(n, vocab_size).astype(np.float32)
        null_result = detect(tokens,n,k,xi_alternative,vocab_size)
...
def detect(tokens,n,k,xi,vocab_size,gamma=0.0,null=False):
    m = len(tokens)
    n = len(xi)
    ...
    for i in range(m-(k-1)):
        for j in range(n):
            A[i][j] = levenshtein(tokens[i:i+k],xi[(j+np.arange(k))%n])

    closest = np.min(A[:,:n],axis=1)[0]
    return np.median(closest)
...
def main(args):
    ...
    t0 = time.time()
    pval = permutation_test(tokens,len(tokenizer),args.key,args.n,len(tokens))
    print('p-value: ', pval)
